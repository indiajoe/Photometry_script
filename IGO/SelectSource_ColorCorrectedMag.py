#!/usr/bin/env python
#USAGE:  ./SelectSource_ColorCorrectedMag.py qphot=1,2,3 source=1 filter=R stds4beta=1,2,3 
import sys
Qphot=[]
Source=[]
Stds=[]
...
def IVsolve(ilineI,ilineV,outFilters):  # outfilters are the filter outputs it should print.
    ...
    for star in Stds :
        if (star in gstars1) and (star in gstars2) :
            ...
            betaVI_array.append( (V-I)-0.863*(v-i) )   # for IGO New CCD
            betaVv_array.append( (V-v)+0.079*(V-I) )
    ...
    if len(betaVI_array) > 0 :
        ...
        for star in Qphot :
            try :
                ...
                VminusI= 0.863*(v-i)+betaVI      # for IGO New CCD
                V= v -0.079*VminusI +betaVv
                I=V - VminusI
...
#            output.append(iline[StartSource+(int(star)-1)*4 +2])
                ...
                output1.append('INDEF')
                output2.append('INDEF')
        if 'I' in outFilters : print(' '.join(output1)+'  B_VI='+str(round(betaVI,3))+'  B_Vv='+str(round(betaVv,3)))
        if 'V' in outFilters : print(' '.join(output2)+'  B_VI='+str(round(betaVI,3))+'  B_Vv='+str(round(betaVv,3)))
    ...
    for star in Stds :
        if (star in gstars1) and (star in gstars2) :
            try :
                ...
                V=StdStarsMag[int(star)-1][2]
                betaVR_array.append( (V-R)-0.883*(v-r) )   # for IGO New CCD
                betaVv_array.append( (V-v)+0.15*(V-R) )
            except ValueError:
                pass
    if len(betaVR_array) > 0 :
        ...
        for star in Qphot :
            try :
                ...
                v=float(ilineV[StartQphot+(int(star)-1)*2])-jd_beta[ilineV[4]]
                VminusR= 0.883*(v-r)+betaVR      # for IGO New CCD
                V= v -0.15*VminusR +betaVv
                R=V - VminusR
...
#        DONE_FLAG='N'
    ...
    if len(iline) > 4 and iline[0][0] != '#' : # and iline[2] in Filter :  #Data line,Not a discarded data line,# Required filter
        if iline[2]== FilterALL[0] : # I image
            ...
            if len(iline_IRV[2]) == 0 : # No V image yet for the night... so simply backup I line.
                iline_IRV_backup[0].append(list(iline))
            else : #  V line already exists 
                outFilters=[]
                if 'I' in Filter : outFilters.append('I')
        ...
        if iline[2]== FilterALL[1] : # R image
            ...
            if len(iline_IRV[2]) == 0 : # No V image yet for the night... so simply backup R line.
